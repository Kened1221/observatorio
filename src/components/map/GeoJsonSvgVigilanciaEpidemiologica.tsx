...
const GeoJsonSvg: React.FC<GeoJSONMapProps> = ({
    provincia,
    distrito,
    setProvincia,
    setDistrito,
    ...
    const getColor = (porcentaje: number) => {
        const percentage = porcentaje * 100;
    ...
    const handleMouseOver = (
        ...
        const distritoInfo = data.find(
            (d) => d.ubicacion.distrito.nombre === distNombre && d.ubicacion.provincia.nombre === provNombre
        );

        const numeroCasos = distritoInfo ? distritoInfo.numeroCasos : 0;
        const evaluados = distritoInfo ? distritoInfo.evaluados : 0;
            ...
            <div className="w-full h-full relative">
                ...
                >
                    {filteredFeatures.map((feature, index) => {
                        ...
                        const distritoActual = data.find(
                            (d) =>
                                d.ubicacion.distrito.nombre === feature.properties.nombdist &&
                                d.ubicacion.provincia.nombre === feature.properties.nombprov
                        );
                ...
                {tooltip.visible && (
                    ...
                    >
                        ...
                        <p>Casos: {tooltip.numeroCasos}</p>
                        <p>Evaluados: {tooltip.evaluados}</p>
                        <p>Porcentaje: {(tooltip.porcentaje * 100).toFixed(1)}%</p>
                    </div>
                )}
            </div>
        </div>
