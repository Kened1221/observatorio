...
export default function ObjetivosPage({ params }: ObjetivosPageProps) {
  ...
  const objectiveNumber = objectiveId.match(/\d+$/)?.[0] || "1";
  const objectiveNameApi = `objetivo ${objectiveNumber}`;
  const objectiveNameDisplay = `Objetivo ${objectiveNumber}`;

  useEffect(() => {
    const fetchOperations = async () => {
      try {
        const ops = await getAvanceOperations(objectiveNameApi);
        setOperations(ops);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const data = await getAvanceData(avance, objectiveNameApi);
        setAvanceData(data);
      } catch (error) {
  ...
  const pieChartData = [
    ...
    {
      name: "50–80% (Amarillo)",
      value: avanceData.filter((d) => d.percentage > 50 && d.percentage <= 80)
        .length,
      fill: "hsl(60, 100%, 50%)",
    },
    {
      ...
      <Card className="shadow-xl border-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100">
            Avances en {objectiveNameDisplay} - Ayacucho
          </CardTitle>
          <div
            className="flex flex-wrap gap-2"
            role="radiogroup"
            aria-label="Selecciona el tipo de avance"
          >
            {operations.map((op) => (
              <button
                key={op}
          ...
          <div className="flex flex-col gap-6">
            <div className="w-full">
              ...
              <ChartContainer
                ...
                aria-label="Gráfico de barras de porcentaje por distrito"
              >
            ...
            <div className="w-full flex justify-center">
              <div className="w-[400px] sm:w-[600px]">
                ...
                <ChartContainer
                  config={chartConfig}
                  className="h-[400px]"
                  aria-label="Gráfico de pastel de distribución de avances"
                >
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
